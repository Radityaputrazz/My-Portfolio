          ...
          return (
            <SwiperSlide key={index}>
              <div className="bg-[rgba(65,47,123,0.15)] h-max rounded-lg px-6 py-8 flex
                ...
                transition-all duration-300 shadow-lg hover:shadow-xl border border-transparent hover:border-accent">
                {/* icons */}
                <div className="text-4xl text-accent mb-4">{item.icon}</div>
                {/* title & description */}
                <div className="mb-8">
                  <div className="mb-2 text-lg font-semibold">{item.title}</div>
                  <p className="max-w-[350px] leading-normal text-gray-700">{item.description}</p>
                </div>
                {/* arrow */}
